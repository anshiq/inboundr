...
export const CRM_NOTE_FILE_MAX_SIZE = 15 * 1024 * 1024

export const CRM_NOTE_IMAGE_TYPES = [
  "image/jpeg",
  "image/png",
  "image/gif",
  "image/webp",
...
export const CRM_NOTE_FILE_ACCEPT = [
  "application/pdf",
  "application/msword",
...
export async function uploadCrmNoteFile(file: File): Promise<NoteAttachment> {
  if (file.size > CRM_NOTE_FILE_MAX_SIZE) {
    const limitMb = Math.round(CRM_NOTE_FILE_MAX_SIZE / 1024 / 1024)
...
// Signed URLs expire, so notes store stable storage keys and every render
// resolves them. The cache keeps a timeline full of images from re-requesting
// a URL per entry per mount; entries are dropped well before S3 expiry.
const VIEW_URL_CACHE_TTL_MS = 1000 * 60 * 60
const viewUrlCache = new Map<string, { url: string; expiresAt: number }>()

export async function resolveCrmFileUrl(key: string): Promise<string> {
...
export async function downloadCrmFile(attachment: NoteAttachment): Promise<void> {
