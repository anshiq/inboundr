...
/**
 * Stored HTML references uploaded images by storage key (`data-key`)
 ...
 */
function useResolvedRichHtml(html: string): string {
    ...
    void (async () => {
      ...
      await Promise.all(
        images.map(async (img) => {
          ...
          if (!key) return
          try {
            img.setAttribute("src", await resolveEditorFileUrl(key))
          } catch {
            img.removeAttribute("src")
...
function AttachmentChip({ attachment }: { attachment: EditorAttachment }) {
  const [downloading, setDownloading] = useState(false)

  async function handleDownload() {
    setDownloading(true)
    try {
      await downloadEditorFile(attachment)
    } catch (err) {
...
export function RichContent({
  ...
  const resolvedHtml = useResolvedRichHtml(html)
  ...
  return (
    <div className={className}>
      <div
        ...
        // Drive/form chips inside the stored HTML open their target on click.
        onClick={(event) => {
          if (handleChipElementClick(event.target as HTMLElement)) {
            event.preventDefault()
          }
        }}
        // Safe to inject: the HTML is sanitized server-side on write.
